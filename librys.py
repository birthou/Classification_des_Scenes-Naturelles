...
class pathDir:
    __train_dir = "/content/drive/MyDrive/Scene_naturelle-classification/DATA/seg_train/seg_train/"
    __test_dir = "/content/drive/MyDrive/Scene_naturelle-classification/DATA/seg_test/seg_test/"
    __pred_dir = "/content/drive/MyDrive/Scene_naturelle-classification/DATA/seg_pred/seg_pred/"
    __pretrained_dir = "/content/drive/MyDrive/Scene_naturelle-classification/PRETRAINEDMODEL/"
...
def dataInfo(img_res=False):
    ...
    sns.barplot(x="category", y="total", palette="YlOrRd", data=df, hue="part")
    plt.ylabel("count")
    plt.xlabel("category")
    plt.title("Images totales de chaque etiquette dans les ensembles de donnees de train et de test")
    plt.show()

    if img_res:
        p = sns.jointplot(dim1, dim2)
        p.fig.suptitle("Resolution moyenne des images")
        p.fig.tight_layout()
